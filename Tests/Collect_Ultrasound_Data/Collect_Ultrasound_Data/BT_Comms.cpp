...
float getBluetoothReading(int n, char measurementType) {
  ...
  if (measurementType == 'd') {
    ...
    serialBT.print(n);
    serialBT.println(" in degrees. Use '-' for angles to the left.");
  }
...
float getBTreading(char measurementType) {
  
  bool waitingForReading = true;
  float reading = 0;

  if (measurementType == 's') {
    serialBT.print("Enter steering");
  }

  if (measurementType == 'p') {
    serialBT.print("Enter power");
  }
  if (measurementType == 't') {
    serialBT.print("Enter time (ms)");
  }
