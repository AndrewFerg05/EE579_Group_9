...
double sendUltrasoundPing(int triggerPin, int echoPin)
{
  // Set up pins
...
Target scanForTargets_Ultrasound() {
  ...
  while (closestTarget.angleToTarget == -1 and scanCount < 10){
    ...
    while(1) {
      ...
      if (leftmostAngle - rightmostAngle <= 15) {
        ...
        closestTarget.distance = distanceReadings[int(closestTarget.angleToTarget)];
        ...
        Serial.print(closestTarget.distance);
        Serial.print("  at an angle: ");
        Serial.println(int(closestTarget.angleToTarget));
    }
  ...
  if (closestTarget.angleToTarget != -1) {
    for (int i = 180; i >= int(closestTarget.angleToTarget); i--) {
      // for (int i = 180; i >= 0; i--) {
    ultrasoundServo.write(ultrasoundServoPin, i);
    delay(5);
  }
