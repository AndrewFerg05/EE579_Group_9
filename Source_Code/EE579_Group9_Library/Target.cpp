...
{
  target->timeToTarget = (int)(((target->distance / car_speed) * 1000) - time_offset); // Tune car speed                                                                                            // Replace with your actual conversion
}
...
double sendUltrasoundPing()
{
  ...
  while (digitalRead(echoPin) == LOW)
  {
    ...
    { // if the pulse doesn't return within the time frame, return 5s
      return 50000;
    }
  };
...
Target scanForTargets_Ultrasound()
{
...

  {
    ...
    while (!processingComplete)
    {
      ...
      {
        if (targetAngle == -1)
        { // if the target angle has not been set, set it
        ...
        { // if target angle has already been set, check if the new distance is smaller
      ...
      else if (objectGradient > 0.1 or objectGradient < -0.1)
      {
        Serial.println("False Reading: Angle range was too small");
        lowerBoundary = distanceReadings[targetAngle];
      }
